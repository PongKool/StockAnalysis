...
# Initialize Anthropic Claude Client (automatically reads ANTHROPIC_API_KEY from environment)
...
# 3. REQUEST STRUCTURED ANALYSIS FROM CLADE API (VALUE & CAP-PRESERVATION LOGIC)
prompt = f"""
...
CRITICAL INSTRUCTION: You must reply ONLY with a valid, clean JSON array of objects. Do not wrap it in markdown code blocks, and do not include any extra text. Each object in the JSON array must follow this exact schema:
{{
  "stock_name": "TICKER",
...
# Cleanup safety check if model outputs markdown blocks anyway
...
class CorporatePDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, "Daily Thai Market Report (Claude Edition)", new_x="LMARGIN", new_y="NEXT", align="L")
...
filename = "thai_market_analysis_claude.pdf"
pdf.output(filename)
print(f"Thai portfolio analysis finalized successfully via Claude API as {filename}.")
